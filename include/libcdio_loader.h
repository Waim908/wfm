...
#include <stdint.h>
    ...
    typedef uint8_t (*cdio_get_joliet_level_fn)(const CdIo_t*);
    typedef driver_return_code_t (*cdio_read_data_sectors_fn)(const CdIo_t*, void*, lsn_t, uint16_t, uint32_t);
    
    typedef iso9660_t* (*iso9660_open_ext_fn)(const char*, iso_extension_mask_t);
    typedef bool (*iso9660_close_fn)(iso9660_t*);
    typedef uint8_t (*iso9660_ifs_get_joliet_level_fn)(iso9660_t*);
    ...
    typedef long (*iso9660_iso_seek_read_fn)(const iso9660_t*, void*, lsn_t, long);
    typedef int (*iso9660_name_translate_ext_fn)(const char*, char*, uint8_t);
    typedef void (*iso9660_filelist_free_fn)(CdioISO9660FileList_t*);
    
    typedef CdioListNode_t* (*_cdio_list_begin_fn)(const CdioList_t*);
    typedef void* (*_cdio_list_node_data_fn)(const CdioListNode_t*);
    typedef CdioListNode_t* (*_cdio_list_node_next_fn)(CdioListNode_t*);
    ...
    static inline BOOL libcdio_load(void) { 
        return TRUE; 
    }
    static inline void libcdio_free(void) { 
        
    }
    static inline BOOL libcdio_is_loaded(void) { 
        return TRUE; 
    }
